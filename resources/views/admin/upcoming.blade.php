...
<hr class="">
<table id="table" class="table table-striped display" cellspacing="0" width="100%" style="font-size: 12px;">
    <thead>
    <tr>
    ...
    @if(count($appointments)>0)
        @foreach ($appointments as $ap)
            <tr id="appointment/{{ $ap->appointment_id }}">
                ...
                <td>{{ $ap->order_from == 'A' ? 'App' : 'Web' }}</td>
            </tr>
        @endforeach
    @else
        <tr>
            <td colspan="15" class="text-center">No upcoming appointment</td>
        </tr>
    @endif
    </tbody>
</table>
