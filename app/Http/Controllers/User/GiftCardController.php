...
class GiftCardController extends Controller
{
    ...
    public function show(Request $request) {
        ...
        $giftcards = Giftcard::whereIn('status', ['A', 'P'])->where('type','K')->orderBy('amt', 'asc', 'id', 'asc' )->get();

        return view('user.gift-cards', [
            'giftcards' => $giftcards
        ]);
    }
    ...
    public function buy(Request $request) {
        ...
        return view('user.gift-cards.buy', [
            'payments'  => $payments,
            'payment'   => $payment
        ]);
    }
    ...
    public function payment(Request $request) {
        ...
        return view('user.gift-cards.payment')->with([
            'years'     => $years,
            'months'    => $months,
            'states'    => $states,
            'voucher'   => $voucher
    ...
    public function buy_process(Request $request) {
        try {
            ...
            if (empty($giftcard) || ($giftcard->status != 'A') || ($giftcard->type != 'K') )  {
                return response()->json([
                    'msg' => 'The Gift card is not available. [GC-NIL]'
                ]);
            }

            //$code = PromoCode::generate_code_for_giftcard($giftcard->amt, $user->user_id);
            $code = PromoCode::generate_code_for_giftcard($giftcard->amt, $user->user_id, $giftcard->id );
            ...
            if (!empty($ret['msg'])) {
                if ($ret['code'] != 1) {
                    ...
                    Helper::send_mail('tech@groomit.me', '[Groomit][' . getenv('APP_ENV') . '] Credit card processing failed', ' - Giftcard sales : ' . $sales->id . '<br> - error : ' . $ret['error_msg']);
                    Helper::send_mail('help@groomit.me', '[Groomit][' . getenv('APP_ENV') . '] Credit card processing failed', ' - Giftcard sales : ' . $sales->id . '<br> - error : ' . $ret['error_msg']);
            ...
            if (!empty($ret['error_msg'])) {
                ...
                $msg = 'Giftcard sales processing failed : ' . $ret['error_msg'] . ' [' . $ret['error_code'] . ']';
            ...
            if ($sales->is_gift == 'Y') {
                ...
                Helper::send_html_mail('vouchers.voucher-gift', $data);

                $data['email']  = $user->email;
                $data['subject'] = 'Thank you for your order, the Groomit Giftcard Code was sent to your recipient';

                Helper::send_html_mail('vouchers.voucher-gift-receipt', $data);

                ## send SMS to User ##
                if (!empty($user->phone)) {
                    $user_message = 'Your card was successfully sent to ' . $data['name'];
                    $ret = Helper::send_sms($user->phone, $user_message);
                    if (!empty($ret)) {
                        //throw new \Exception('Groomer SMS Error: ' . $ret);
                        Helper::send_mail('tech@groomit.me', '[groomit][' . getenv('APP_ENV') . '] SMS Error:' . $ret, $user_message . '/ Giftcard Sales ID:' . $sales->id);
                    }
                ...
                $data['email']  = $user->email;
                $data['subject'] = 'Thank you for your order, here is your Groomit Giftcard Code';

                Helper::log('##### EMAIL DATA #####', [
                    'data' => $data
                ]);

                Helper::send_html_mail('vouchers.voucher-receipt', $data);
            }

            $data['email']  = 'it@jjonbp.com';
            $data['subject'] = '[Giftcard] Order by [' . $user->user_id . '] ' . $user->email . ' !! $' . $sales->amt;
