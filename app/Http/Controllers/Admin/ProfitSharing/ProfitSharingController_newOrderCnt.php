...
//            ->leftJoin('appointment_cnt', 'appointment_cnt.appointment_id', '=', 'appointment_list.appointment_id') ;
//
...
//                $data = $data->where(DB::raw('IfNull(appointment_cnt.paid_cnt,0)') , '<=', DB::raw('1') );
//            } else {
//                # repeating user
//                //$data = $data->where(DB::raw('f_get_appointment_cnt(a.user_id)'), '>', DB::raw('1'));
//                $data = $data->where(DB::raw('IfNull(appointment_cnt.paid_cnt,0)') , '>', DB::raw('1') );
...
//            //This is based on paid_cnt, not booked_cnt. //f_get_appointment_cnt()
//            if ($request->repeating == 'N') {
//                # 1st time user
//                $data = $data->where(DB::raw('IfNull(appointment_cnt.paid_cnt,0)') , '<=', DB::raw('1') );
//            } else {
...
//                          'Groomer.Fav.Groomer.Earning' => $a->groomer_fav_earning,
//                          'Groomer.Profit.Amt' => $a->groomer_profit_amt,
//                          'Groupon.Payout'  => $a->type == 'T' ? '0.00' : $a->groupon_amt,
//                          'Profit'          => $a->profit_amt,
//                          'Last.Updated'    => $a->cdate
...
//                      'Groomer.Fav.Groomer.Earning' => $total->sum_groomer_fav_earning_amt,
