...
class SendReminderEmailCA extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:reminder_ca';
     ...
     * @var string
     */
    protected $description = 'Send Reminder Emails : CA';
                ...
                foreach ($apps as $app) {
                    ...
                    if ($tomorrow_reminder) {
                        ...
                        if (!empty($user->device_token)) {
                            ...
                            if (!empty($error)) {
                                Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Push Notification failed - reminder tomorrow', 'Push Notification Error: ' . $error);

                            }
                        } else {
                            Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Push Notification failed - reminder tomorrow', 'Push Notification Error: No device token found');
                        }
                        ...
                        if (!empty($user->phone)) {
                            ...
                            if (!empty($ret)) {
                                Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Send user SMS failed : reminder tomorrow - User ID: ' . $app->user_id . ', Appointment ID : ' . $app->appointment_id, $ret);
                            }
                        ...
                        if (!empty($groomer->mobile_phone)) {
                            ...
                            $ret = Helper::send_sms($phone, $message);
                            if (!empty($ret)) {
                                Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Send groomer SMS failed - reminder tomorrow - Groomer ID: ' . $app->groomer_id . ', Appointment ID : ' . $app->appointment_id, $ret);
                            }
                        }
                        ### end send text ###
                        ...
                        if (!empty($user->device_token)) {
                            ...
                            $error = Helper::send_notification('groomit', $message, $user->device_token, $subject, $payload);
                            if (!empty($error)) {
                                Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Push Notification failed - reminder today', 'Push Notification Error: ' . $error);

                            }
                        } else {
                            Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Push Notification failed - reminder today', 'Push Notification Error: No device token found');
                        }
                        ...
                        if (!empty($user->phone)) {
                            ...
                            $ret = Helper::send_sms($phone, $message);
                            if (!empty($ret)) {
                                Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Send user SMS failed : reminder today - User ID: ' . $app->user_id . ', Appointment ID : ' . $app->appointment_id, $ret);
                            }
                            ...
                            if (!empty($ret)) {
                                Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Send user SMS failed : Covid19 Intro: ' . $app->user_id . ', Appointment ID : ' . $app->appointment_id, $ret);
                            }
                        ...
                        if (!empty($groomer->mobile_phone)) {
                            ...
                            $ret = Helper::send_sms($phone, $message);
                            if (!empty($ret)) {
                                Helper::send_mail($tech_email, '[Groomit][' . getenv('APP_ENV') . '] Send groomer SMS failed - reminder today - Groomer ID: ' . $app->groomer_id . ', Appointment ID : ' . $app->appointment_id, $ret);
                            }
                        }
                        ### end send text ###
